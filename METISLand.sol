...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    // mapp yield info
    mapping(uint256 => Info) public metisLand;
    ...
    // claim yield
    function claimmetisLand(uint256 _tokenId) public {
        require(_tokenId > 0, "can only claim NFT");
        require(balanceOf(msg.sender, _tokenId) == 1, "only owner can claim");
        require(block.timestamp > metisLand[_tokenId].date, "too early to claim");
        require(metisLand[_tokenId].canClaim == true, "already claimed");
        metisLand[_tokenId].canClaim = false;
        _operatorApprovals[address(this)][msg.sender] = true;
        safeTransferFrom(
            address(this),
            msg.sender,
            0,
            metisLand[_tokenId].level,
            "0x"
        );
    }
    ...
    function sellOrder(uint256 _tokenId, uint256 _weiAmt) public {
        ...
        metisLand[_tokenId].owner = msg.sender;
        metisLand[_tokenId].canTrade = true;
        metisLand[_tokenId].weiAmt = _weiAmt;
        // 20% fee
        metisLand[_tokenId].weiFee = div(_weiAmt, 5);
    ...
    function cancelOrder(uint256 _tokenId) public {
        ...
        require(msg.sender == metisLand[_tokenId].owner, "only owner can cancel");
        // canceled orders can't claim
        metisLand[_tokenId].canClaim = false;
        metisLand[_tokenId].canTrade = false;
        // send back to owner
    ...
    function buyOrder(uint256 _tokenId) public payable returns (bytes memory) {
        require(metisLand[_tokenId].canTrade == true, "not tradable");
        require(msg.sender != metisLand[_tokenId].owner, "owner cannot buy");
        require(
            msg.value == add(metisLand[_tokenId].weiAmt, metisLand[_tokenId].weiFee),
            "invalid amount"
        );
        // prevent re-entry
        metisLand[_tokenId].canTrade = false;
        ...
        (bool sent, bytes memory data) = metisLand[_tokenId].owner.call{
            value: metisLand[_tokenId].weiAmt
        }("");
    ...
    function ownerMint() public {
        ...
        tokenCount++;
        metisLand[tokenCount].owner = msg.sender;
        metisLand[tokenCount].level = 1;
        metisLand[tokenCount].canClaim = true;
    ...
    function levelUp(uint256 _tokenId) public {
        ...
        require(metisLand[_tokenId].owner == msg.sender, "only owner can level up");
        // burn amount = current level * 2
        uint256 burnAmount = metisLand[_tokenId].level * 2;
    ...
    function _safeTransferFrom(
        ...
        metisLand[id].date = block.timestamp + metisLand[id].level * 1 seconds;
        // set claim to true
        metisLand[id].canClaim = true;
